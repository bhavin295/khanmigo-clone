...
export async function generateAIJson<T>(
  messages: ChatMessage[],
  systemPrompt: string
): Promise<{ data: T | null; raw: string; errors: string[] }> {
...
export async function analyzeImageWithAI(
  ...
  data: VisionAnalysisResult | null;
  raw: string;
  errors: string[];
}> {
  const prompt =
    'Extract the text from this homework problem or question. Identify the subject. Return JSON: { "extracted_text": string, "detected_subject": string, "description": string }';
  const errors: string[] = [];
  let raw = "";
...
export function coerceSubject(input?: string | null) {
  if (input === "reading" || input === "science" || input === "writing")
    return input;
  return "math";
}
