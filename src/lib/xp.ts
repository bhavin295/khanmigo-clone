...
export async function checkAndAwardAchievements(supabase: any, userId: string) {
  ...
  if (conversationIds.length > 0) {
    ...
    if ((msgs ?? 0) >= 50)
      await grantAchievement(supabase, userId, "messages_50");
    if ((msgs ?? 0) >= 200)
      await grantAchievement(supabase, userId, "messages_200");
  }
  ...
  if ((xpData?.current_streak ?? 0) >= 3)
    await grantAchievement(supabase, userId, "streak_3");
  if ((xpData?.current_streak ?? 0) >= 7)
    await grantAchievement(supabase, userId, "streak_7");
}
...
async function grantAchievement(supabase: any, userId: string, code: string) {
  ...
  if (!error) {
    const { data: ach } = await supabase
      ...
      .single();
    if (ach?.xp_reward)
      await awardXP(supabase, userId, "achievement", ach.xp_reward, code);
  }
}
