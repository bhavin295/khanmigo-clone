...
export function ImageSolver({
  ...
  const subjectLabel = useMemo(() => {
    ...
    return `Looks like ${analysis.detected_subject[0]?.toUpperCase()}${analysis.detected_subject.slice(
      1
    )}`;
  }, [analysis]);
  ...
  return (
    ...
    >
      {!file ? (
        ...
        >
          ...
          <p className="mt-1 text-xs" style={{ color: "var(--text-muted)" }}>
            Supports handwritten & printed problems
          </p>
          <button
            type="button"
            onClick={open}
      ...
      {analysis ? (
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              type="button"
              onClick={() => onAnalysis(analysis.extracted_text)}
              className="rounded-xl px-4 py-2 text-sm font-semibold text-white"
              style={{ background: "var(--grad)" }}
            >
              Send to tutor →
      ...
      {error ? (
        <p className="text-xs" style={{ color: "rgba(252,165,165,1)" }}>
          {error}
        </p>
      ) : null}
    </div>
